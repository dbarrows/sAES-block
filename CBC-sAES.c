...
int main(int argc, char *argv[]) {
    ...
    //get pad length if provided, default is no padding
    int pad_length = 0;
    if(argc >= 6)
        pad_length = atoi(argv[5]);
    ...
    word IV = 0x1f65; //default IV
    if(argc == 7) {
        int nonce = atoi(argv[6]);
    ...
    if (encrypt_flag) {
        ...
        output[0] = encrypt_word( IV^data[0] , key);                //encrypt
        for(i = 1; i < num_blocks; i++)
            output[i] = encrypt_word(output[i-1]^data[i], key);
        ...
        output[0] = decrypt_word(data[0], key) ^ IV;                //decrypt
        for(i = 1; i < num_blocks; i++)
            output[i] = decrypt_word(data[i], key) ^ data[i-1];
